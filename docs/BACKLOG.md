...
| ID | 类型 | 优先 | 事项 | 下一步 | 详情 |
| --- | --- | --- | --- | --- | --- |
| PROB-36 | 技术债 | P3 | 首页搜索未按性能契约 `C-5`（settle 前 mutation = 0）防抖：快速连打时 settle 前就多次重渲染。2026-09-19 L3 `perf:audit` 实测连打 `n`/`np`/`npm` 后 settle 前 **16** 次 mutation（其余 9 项预算通过）。与已发布的 PROB-24 / PROB-35 无关——两者均未触碰首页搜索链路 | 先确认是真实缺防抖还是测量临界（复跑并核对 `Home` / `HomeHeroSearch` 搜索输入的节流实现）；若确为缺防抖，给首页搜索输入加防抖，抽纯函数写单测，再复跑 `perf:audit` 确认 `C-5` 归零 | — |
