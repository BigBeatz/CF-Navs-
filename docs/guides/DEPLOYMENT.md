...
- 进入 **站点设置 → 账号安全**，输入当前密码更新管理员密码。成功后现有会话会失效，需要使用新密码重新登录。

**第二层：无法登录，但能操作 Cloudflare Secret**

- 打开站点 `/recover`，输入当前生产环境的 `SETUP_TOKEN` 和新密码。恢复端点只修改密码，不修改管理员用户名；新密码须为 8–12 位，且至少包含两类字符。
- 忘记了原 `SETUP_TOKEN`：在 Worker **设置 → 变量和密钥 → 生产环境** 新增或编辑类型为**密钥**的 `SETUP_TOKEN`，保存后重新触发生产分支部署，再使用新值访问 `/recover`。令牌不存 D1，修改后的环境值会成为下一次请求的校验值。
- `/recover` 成功后会轮换 JWT secret，所有旧会话失效，并直接返回登录态。

**第三层：无法登录，也不方便使用浏览器恢复页**

- 以下 `INIT_ADMIN_*` 流程仅用于已完成初始化的旧数据库升级或凭据恢复，不适用于全新部署。修改 `INIT_ADMIN_USER` 和 `INIT_ADMIN_PASSWORD` 后重新部署，下一次登录会自动用新值覆盖 D1 中的管理员凭据。确认当前 Wrangler 指向正确的 Worker、D1 和账号后再执行：
...
- 已经存在但升级前创建的旧数据库可能还没有初始化标记。此时再增加一个新的 `RESET_ADMIN_CREDENTIALS` 变量值，例如 `reset-2026-07-12`，重新部署并用新凭据登录一次。成功后可以移除该变量；同一个标记不会重复重置，以后再次强制重置时请使用新的标记值。

若完全没有 Cloudflare 后台权限，也无法重新部署，则没有可用的应用层恢复路径；这是部署者权限的固有边界。
